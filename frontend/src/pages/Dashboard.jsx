...
import {
  ...
  Wallet, PackageX, Sparkles, MessageSquareText, Calendar, ArrowUpRight
} from "lucide-react";
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <header className="sticky top-0 z-20 backdrop-blur-md bg-surface/80 border-b border-line -mx-6 -mt-6 px-6 py-3.5 mb-6 flex items-center justify-between transition-all">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5 pl-2 border-l border-line">
            ...
            <span className="text-xs font-medium text-ink/80 hidden sm:inline">{user?.full_name}</span>
          </div>
        </div>
      </header>
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-white/10 backdrop-blur-md text-teal-200 text-xs font-medium mb-2 border border-white/10">
              <Sparkles size={13} /> Real-time System Overview
            </div>
            <h1 className="font-display text-2xl sm:text-3xl font-semibold tracking-tight">
              Good day, {user?.full_name?.split(" ")[0]}
            </h1>
            <p className="text-teal-100/70 text-sm mt-1">Here is what is happening across the facility today.</p>
          </div>
        </div>
      </div>
        ...
        <Card className="border border-line/60 shadow-sm rounded-2xl overflow-hidden">
          <div className="p-4 border-b border-line flex items-center justify-between bg-surface/50">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-medium text-base">Recent Referrals & Messages</h3>
            </div>
            <Link
              to="/messages"
          ...
          <div className="divide-y divide-line/60">
            {messages.map((m) => (
              <div key={m.id} className="p-3.5 hover:bg-line/20 transition-colors flex items-center gap-3">
                <div className="w-2 flex justify-center">
                  {!m.is_read && <span className="h-2 w-2 rounded-full bg-teal-500 animate-pulse" />}
                </div>
                <div className="min-w-0 flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-sm">
                  <div className="flex items-center gap-2 truncate">
                    <Badge tone="neutral" className="text-xs font-mono">{m.from_department}</Badge>
                    <span className="font-medium text-ink truncate">{m.patient?.full_name}</span>
                  </div>
                  {m.reason && <span className="text-xs text-ink/50 truncate max-w-xs">{m.reason}</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Main Analytics Line Charts Section */}
      <div className="grid md:grid-cols-2 gap-6">
        {/* Visit Volume Smooth Area/Line Chart */}
        <Card className="p-5 border border-line/60 rounded-2xl shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-display text-lg font-medium">Patient Visit Volume</h3>
              <p className="text-xs text-ink/50">Daily encounters recorded over the last 7 days</p>
            </div>
          </div>
            ...
            <AreaChart data={data.visits_last_7_days ?? []} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
              <defs>
                <linearGradient id="visitGradient" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <stop offset="95%" stopColor="#0F4C4A" stopOpacity={0.0} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgb(var(--color-line))" opacity={0.5} />
              <XAxis dataKey="date" tickLine={false} tick={{ fontSize: 11, fill: "rgb(var(--color-ink) / 0.5)" }} tickFormatter={(d) => d.slice(5)} />
              <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: "rgb(var(--color-ink) / 0.5)" }} allowDecimals={false} />
              <Tooltip
                contentStyle={{
              ...
              />
              <Area type="monotone" dataKey="count" stroke="#0F4C4A" strokeWidth={3} fillOpacity={1} fill="url(#visitGradient)" />
            </AreaChart>
          </ResponsiveContainer>
        </Card>

        {/* Department Volume Smooth Line Chart */}
        <Card className="p-5 border border-line/60 rounded-2xl shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-display text-lg font-medium">Active Queue by Department</h3>
              <p className="text-xs text-ink/50">Current live distribution across units</p>
            </div>
          </div>
          {deptData.length === 0 ? (
            <div className="h-[240px] flex items-center justify-center">
              <p className="text-sm text-ink/40">No active department queues right now.</p>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={deptData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgb(var(--color-line))" opacity={0.5} />
                <XAxis dataKey="name" tickLine={false} tick={{ fontSize: 11, fill: "rgb(var(--color-ink) / 0.5)" }} />
                <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: "rgb(var(--color-ink) / 0.5)" }} allowDecimals={false} />
                <Tooltip
                  contentStyle={{
                    background: "rgba(var(--color-surface), 0.95)",
                ...
                <Line
                  ...
                  stroke="#3F8C84"
                  strokeWidth={3}
                  dot={{ r: 4, fill: "#3F8C84", strokeWidth: 2, stroke: "#ffffff" }}
      ...
      {/* Priority Breakdown Hollow Donut */}
      {priorityData.length > 0 && (
        <Card className="p-5 border border-line/60 rounded-2xl shadow-sm md:w-1/2">
          <div>
            <h3 className="font-display text-lg font-medium">Encounter Priority Breakdown</h3>
            <p className="text-xs text-ink/50">Categorization by urgency level</p>
          </div>
              ...
              <PieChart>
                ...
                >
                  {priorityData.map((entry, i) => (
                    <Cell key={entry.name} fill={COLOR_PALETTE[i % COLOR_PALETTE.length]} cornerRadius={4} />
                  ))}
                </Pie>
                <Tooltip
                  contentStyle={{
              ...
              {priorityData.map((item, index) => (
                <div key={item.name} className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-ink/70 capitalize">{item.name}</span>
                  </div>
...
function StatCard({ label, value, tone, icon: Icon, trend }) {
  const iconTones = {
    neutral: "bg-teal-500/10 text-teal-600 dark:text-teal-400",
  ...
  return (
    <Card className="p-4 border border-line/60 rounded-2xl hover:border-teal-500/30 transition-all duration-200 hover:-translate-y-0.5 shadow-sm">
      <div className="flex items-start justify-between mb-3">
        <p className="text-xs font-medium text-ink/60 leading-snug pr-2">{label}</p>
        {Icon && (
          <div className={`h-8 w-8 rounded-xl flex items-center justify-center shrink-0 ${iconTones[tone || "neutral"]}`}>
      ...
      <div className="flex items-baseline justify-between">
        ...
        {trend && <span className="text-[10px] font-semibold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-md">{trend}</span>}
      </div>
      {tone && tone !== "neutral" && tone !== "teal" && (
        <div className="mt-2.5">
          <Badge tone={tone}>
            {tone === "critical" ? "Needs Attention" : tone === "warning" ? "Monitor" : "Normal"}
          </Badge>
        </div>
      )}
    </Card>
