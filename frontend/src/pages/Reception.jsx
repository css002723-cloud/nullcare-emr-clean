...
import { ClipboardList, UserPlus, UserCheck, ListOrdered, Siren, X, Clock } from "lucide-react";
import api from "../services/api";
import { Card, Field, Input, Select, Button, Badge, Textarea, LoadingRow, EmptyState } from "../components/ui";
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-sm font-medium transition-colors ${
              tab === t.value ? "bg-teal-500 text-white" : "bg-surface border border-line text-ink/60"
            }`}
          >
    ...
    return (
      <Card className="border-moss/30 bg-moss/5">
        ...
        <p className="text-sm mt-1">
          {result.patient.given_name} {result.patient.family_name} — Patient ID:{" "}
          <span className="mrn-mono">{result.offline ? "pending sync" : result.patient.patient_uid}</span>
        </p>
        <p className="text-sm">This visit's MRN: <span className="mrn-mono">{result.offline ? "pending sync" : result.encounter.mrn}</span></p>
        {result.offline && (
          <p className="text-xs text-clay mt-2">
            This record is queued and will get its official patient ID and visit MRN once your connection is
            restored and the sync completes.
          </p>
        )}
        <div className="flex gap-2 mt-3">
          {!result.offline && (
            <Button size="sm" onClick={() => navigate(`/patients/${result.patient.patient_uid}`)}>
              Open patient record
            </Button>
          )}
        ...
        {duplicates.length > 0 && (
          <div className="mt-4 bg-clay/10 border border-clay/25 rounded-lg p-3">
            ...
            <ul className="text-sm mt-1 space-y-1">
              {duplicates.map((d) => (
                <li key={d.id}>
                  {d.full_name} — <span className="mrn-mono">{d.patient_uid}</span>{" "}
                  <button
                    type="button"
            ...
            <p className="text-xs text-ink/50 mt-1">
              If this is the same person, use the "Returning patient" tab instead of creating a duplicate.
            </p>
          </div>
        )}
      </Card>
...
function VisitDetailsCard({ form, update }) {
  return (
    <Card>
      ...
      {(form.visit_type === "emergency" || form.priority === "emergency") && (
        <p className="mt-3 text-xs text-alert bg-alert/5 border border-alert/20 rounded-lg px-3 py-2 flex items-center gap-1.5">
          <Siren size={13} /> This visit will be flagged as an emergency and prioritized at the top of every queue.
        </p>
      )}
    </Card>
  );
...
function ReturningPatientPanel() {
  ...
  const [visit, setVisit] = useState({ chief_complaint: "", visit_type: "outpatient", priority: "routine" });
  const [submitting, setSubmitting] = useState(false);
  const [result, setResult] = useState(null);
  const [error, setError] = useState("");
  ...
  async function activate(e) {
    ...
    setError("");
    if (!patientId) {
      setError("Search for and select the returning patient first.");
      return;
    }
    setSubmitting(true);
    try {
      const { data: encounter, offline } = await createWithOfflineFallback("encounter", "/encounters", {
        patient_id: patientId, ...visit,
      });
      setResult({ encounter, offline, patientLabel });
      setPatientId(null);
      setPatientLabel("");
      setVisit({ chief_complaint: "", visit_type: "outpatient", priority: "routine" });
      setResetKey((k) => k + 1);
    } catch (err) {
      setError(err.response?.data?.message || "Couldn't open a new visit for this patient — please try again.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (result) {
    return (
      <Card className="border-moss/30 bg-moss/5">
        <p className="font-semibold text-moss">Patient activated for today's visit{result.offline ? " (saved offline)" : ""}</p>
        <p className="text-sm mt-1">{result.patientLabel}</p>
        <p className="text-sm">
          New visit MRN: <span className="mrn-mono">{result.offline ? "pending sync" : result.encounter.mrn}</span>
          {" "}— their permanent patient ID has not changed.
        </p>
        <p className="text-xs text-ink/50 mt-1">The patient now moves to triage and nursing, exactly like a new arrival.</p>
        <Button size="sm" variant="secondary" className="mt-3" onClick={() => setResult(null)}>
  ...
  return (
    <form onSubmit={activate} className="space-y-5">
      <Card>
        <p className="font-display text-lg mb-1">Find the returning patient</p>
        <p className="text-xs text-ink/50 mb-3">
          No new registration needed — search by name or their permanent patient ID. Their record, allergies, and
          visit history all carry over; only a fresh visit MRN is generated for today.
        </p>
        <PatientLookup
          key={resetKey}
          label="Patient"
          onSelect={({ patientId, patientLabel }) => { setPatientId(patientId); setPatientLabel(patientLabel || ""); }}
        />
      </Card>

      <VisitDetailsCard form={visit} update={(field, value) => setVisit((v) => ({ ...v, [field]: value }))} />

      {error && <p role="alert" className="text-sm text-alert">{error}</p>}

      <Button type="submit" size="lg" disabled={submitting}>
        {submitting ? "Activating…" : "Activate patient & open visit"}
      </Button>
    </form>
  );
}
...
function QueuePanel() {
  ...
  async function cancelEntry(encounterId) {
    if (!window.confirm("Cancel this registration? Use this only for a mistaken entry or a no-show.")) return;
    await api.post(`/encounters/${encounterId}/close`, { outcome: "cancelled", disposition_notes: "Cancelled from reception queue" });
    load();
  }
  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-3">
        <p className="font-display text-lg">Today's queue</p>
        <Badge tone="muted">{encounters.length} waiting</Badge>
      </div>

      {loading ? <LoadingRow /> : encounters.length === 0 ? (
        <EmptyState icon={ListOrdered} title="No one waiting right now" hint="New arrivals and returning patients will appear here." />
      ) : (
        <ul className="divide-y divide-line">
          {encounters.map((e) => (
            <li key={e.id} className={`py-3 flex items-center justify-between flex-wrap gap-2 ${e.is_emergency ? "bg-alert/5 -mx-2 px-2 rounded" : ""}`}>
              <div className="min-w-0">
                <p className="font-medium flex items-center gap-1.5">
                  {e.is_emergency && <Siren size={14} className="text-alert shrink-0" />}
                  {e.patient?.full_name}
                  <span className="text-xs text-ink/40 font-normal mrn-mono">({e.mrn})</span>
                </p>
                <p className="text-xs text-ink/50">{e.chief_complaint || "No complaint noted"}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-xs text-ink/45 flex items-center gap-1"><Clock size={12} /> {waitTime(e.created_at)}</span>
                <Badge tone="muted">{e.current_department}</Badge>
                {e.is_emergency ? (
                  <Badge tone="critical" icon={Siren}>emergency</Badge>
                ) : (
                  <Select value={e.priority} onChange={(ev) => updatePriority(e.id, ev.target.value)} className="text-xs py-1 w-auto">
                    <option value="routine">Routine</option>
                    <option value="urgent">Urgent</option>
                    <option value="emergency">Emergency</option>
                  </Select>
                )}
                <Button size="sm" variant="ghost" icon={X} onClick={() => cancelEntry(e.id)} aria-label="Cancel this registration">
                  Cancel
                </Button>
              </div>
            </li>
